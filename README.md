...
## Введение
Добро пожаловать в документацию SDK! Этот SDK предоставляет возможность встроить функционал **атомарного обмена WhiteBird** прямо к себе в мобильное приложение(Swift/Kotlin SDK) или на сайт(JS SDK).
...
Вторая возможность, которую это открывает - использование SDK в TokensMode. Пользователю не понадобится раз авторизовываться на платформе WhiteBird самостоятельно, это за него сделает клиент, получив Auth tokens через **backend to backend** взаимодействие по REST API, c помощью API Key мерчанта.
...
wbExchangeSdk.setup({
  ...
  // optional params:
  // позволяет показать кнопку "back" на нашем UI 
  showBackButtonOnHomePage: false,
  // callback для исполнения действий по нажатию на "back"
  onExit: () => {},
  // Убирает возможность пользователю добавлять карты через WhiteBird
